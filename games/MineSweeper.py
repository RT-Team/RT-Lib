...
class Ms:
    def __init__(self,mx,my,bomb,log=False):
        ...
        self.check_end = lambda n:self.mx*self.my-self.bomb==len(sum([[x for x in y if not x in self.objs]for y in n],[]))
    ...
    def back_get(self,l,margin):
        r = " "+margin*2+margin.join([str(x+1) for x in range(self.mx)])+"\n"
    ...
    def rep(self,x,y,z,mode=0):
        ...
        for i in range(2):
            for cx,cy in c:
                ...
                if self.log:print(f"  Set {cx} {cy}")
                try:
                    if self.b[cy][cx] == self.objs[0]:
                        bombs += 1
                    self.now[y][x] = str((self.objs[2] if self.now[y][x] != self.objs[2] else "-") if z else int(bombs/2))
                    self.b[y][x] = str((self.objs[2] if self.b[y][x] != self.objs[2] else "-") if z else int(bombs/2))
                except IndexError:
                    continue
            if bombs == 0 and bombs != self.objs[2]:
                for cx,cy in c:
                    if self.log:print(f"  Set {cx} {cy}")
                    if cx == -1 or cy == -1 or self.my <= cy or self.mx <= cx or [cx,cy] in self.did:continue
    ...
    def set(self,x,y,z=False):
        ...
        if len(self.b) <= y:return 200
        if len(self.b[0]) <= x:return 200

        sl = self.b[y][x]
        if sl == self.objs[1]:
            self.rep(x,y,z)
            if self.check_end(self.b) and not z:return 301
            else:return 200
